...
function SuggestionProfile({ User }) {
  const { user, profiledata, hendletoFollow, followers } =
  ...
  return (
    <div className="flex items-center gap-2 py-2 justify-between xl:max-w-[80%] 2xl:max-w-[70%] px-4">
      ...
      <span
        className="text-blue-700 px-2 rounded-md cursor-pointer hover:text-blue-950"
        onClick={() => hendletoFollow(User._id)}
      >
        {User.followers.includes(user._id.toString()) ? "Unfollow" : "Follow"}
      </span>
    </div>
  );
}
