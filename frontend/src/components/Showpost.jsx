...
function Showpost({ post }) {
  ...
  return (
    <div
      ...
      } w-full sm:max-w-[80%] md:max-w-[80%] lg:max-w-[90%] xl:max-w-[70%] my-4 rounded-md mx-auto`}
    >
      <div
        className={`flex items-center justify-between w-full md:px-8 px-4 pt-0 md:pt-4`}
      >
        <div
        ...
        >
          <div className={`md:text-5xl text-3xl`}>
            {post.postAuthor?.profileImage ? (
              <div className="md:w-12 md:h-12 w-10 h-10 rounded-full overflow-hidden border">
                <img
          ...
          <div className="flex flex-col">
            ...
            <div
              className={`${
                color === "light" ? "text-gray-500" : "text-gray-400"
              } text-sm`}
            >
              {post.postAuthor?.bio}
            </div>
        ...
        {post?.postImage?.map((image, index) => {
          return (
            <img
              ...
              src={image}
              alt=""
              className="w-full object-cover p-2 md:p-4 object-center rounded-md"
            />
          );
        })}
      </div>
      <div className="flex items-center justify-between px-4">
        <div className="flex items-center gap-4">
          <div
            className="text-2xl md:text-3xl cursor-pointer hover:text-slate-400"
            onClick={hendleLike}
          >
            {showLike.includes(user._id) ? (
          ...
          <div
            className="text-2xl md:text-3xl cursor-pointer hover:text-slate-400"
            onClick={hendleShowComment}
          >
            <FaRegComment />
          </div>
          <div className="text-2xl md:text-3xl cursor-pointer hover:text-slate-400">
            <FiSend />
          </div>
        </div>
        <div>
          <div
            className="text-2xl md:text-3xl cursor-pointer hover:text-slate-400"
            onClick={hendlebookmark}
          >
            {showbookmark ? <MdBookmark /> : <FaRegBookmark />}
          </div>
        </div>
      </div>
      <div className="px-4 py-1 md:py-2">{showLike.length} likes</div>
      <div className="px-4 text-sm md:text-base">
        <span
          className={`${
            color === "light" ? "text-black font-bold" : "text-white font-bold"
          }`}
        >
          {post.postAuthor.userName}.
        </span>{" "}
      ...
      <div
        className="px-4 text-slate-400 text-sm md:text-base cursor-pointer"
        onClick={hendleShowComment}
      >
      ...
      <div className="flex items-center px-4 py-2 pb-4">
        <div className="cursor-pointer text-2xl">
          {user?.profileImage ? (
            <div className="md:w-8 md:h-8 w-6 h-6 rounded-full overflow-hidden border">
              <img
        ...
        <input
          ...
          className="w-full px-4 text-sm md:text-base border-none outline-none bg-transparent"
        />
        <div className="cursor-pointer text-blue-500" onClick={hendleComment}>
