...
function Home() {
  const {
    ...
    profiledata,
    notificationCount,
    setnotificationCount} = useContext(StoreContext);
  ...
  const hendleLogout = async () => {
    ...
    try {
      const res = await fetch(`${url}/user/api/user/logout`, {
        method: "GET",
        credentials: "include",
        headers: {
  ...
  const chengeTheam = async () => {
    ...
    try {
      const res = await fetch(`${url}/user/api/user/theam`, {
        method: "POST",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
    ...
    >
      <div className="flex flex-col md:flex-row justify-between h-full">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              >
                ...
                {notificationCount > 0 && (
                  <div className="bg-red-800 text-white absolute -top-2 -right-2 rounded-full text-sm p-2 w-5 h-5 flex justify-center items-center">
                    {notificationCount}
                  </div>
                )}
              </div>
              <div
                className={`text-xl hidden lg:block font-sans ${
                  color === "light" ? "text-black" : "text-white"
                }`}
              >
                <div>
            ...
            >
              ...
              {setting && (
                <div
                  className={`absolute top-7 right-0 w-28 md:w-32 ${
                    color === "light"
                      ? "bg-white"
                      : "bg-slate-800 shadow-slate-500"
                  } shadow-md rounded-md flex flex-col items-center justify-center z-50`}
                >
                  <div className="text-base py-2" onClick={hendleLogout}>
                    Logout
                  </div>
                  ...
                  <div className="text-base py-2" onClick={chengeTheam}>
        ...
        <div
          ...
          } min-w-[0%] max-w-[0%] md:min-w-[15%] md:max-w-[15%] relative hidden md:flex`}
        >
          <LeftSidebar />
          <div
            className={`${
              color === "light" ? "bg-slate-300" : "bg-slate-700"
            } h-full w-px absolute right-0`}
          ></div>
        </div>

        <div
          className={`w-full md:min-w-[85%] md:max-w-[85%] lg:min-w-[60%] lg:max-w-[60%] max-h-[82%] min-h-[82%] md:max-h-[100%] md:min-h-[100%]`}
        >
          <Outlet />
        </div>

        <div
          className={`lg:min-w-[25%] lg:max-w-[25%] hidden lg:block min-w-[0%] max-w-[0%] px-4`}
        >
          <RightSidebar />
        </div>
        ...
        <div className="flex items-center justify-between px-4 md:hidden max-h-[8%] min-h-[8%] md:max-h-[0%] md:min-h-[0%]">
          ...
          >
            ...
            >
              {location.pathname === `/${user._id}/profile` ? (
                user?.profileImage ? (
            ...
            >
              <div>
                {location.pathname === `/${user._id}/profile` ? (
                  ...
                  <span className="tet-lg">Profile</span>
                )}
              </div>
            </div>
          </div>
      ...
      {showcomment && (
        ...
        >
          <ShowComment selectPostcomment={selectPostcomment} />
        </div>
      )}
      {ispopup && (
        <div
