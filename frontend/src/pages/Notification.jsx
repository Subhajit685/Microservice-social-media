...
function Notification() {
  const {
    ...
    setlengthCount,
    notificationLength,
    setnotificationLength} = useContext(StoreContext);
  ...
  useEffect(() => {
    ...
    const setnotification = async () => {
      try {
        const res = await fetch(`${url}/user/api/user/setnotification`, {
          method: "POST",
          credentials: "include",
          headers: {
  ...
  return (
    <div className="w-screen h-full">
      <div className="container w-full h-full mx-auto">
        <h1 className="max-h-[10%] min-h-[10%] text-3xl font-bold py-8 hidden md:block px-4">
          Notification
        </h1>
        ...
        <div className="max-h-[100%] min-h-[100%] overflow-y-scroll">
          {shownotification ? (
            shownotification.map((item, index) => {
              return (
                <div key={index} className="flex gap-4 items-center py-4 px-4">
                  <div
                    className="cursor-pointer"
                    onClick={() => hendleProfile(item?.id)}
                  >
                    {item?.userImage ? (
