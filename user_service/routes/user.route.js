...
route.post("/signup", async (req, res) => {
  try {
    ...
    if (user) {
      return res
        .status(400)
        .json({ success: false, message: "Email all ready exit" });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Internal server error on sign-up" });
  }
});
...
route.post("/login", async (req, res) => {
  try {
    ...
    const token = genTokenSetCookie(user, res);
    ...
    return res
      .status(500)
      .json({ success: false, message: "Internal server error on login" });
  }
});
...
route.get("/logout", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Internal server error on logout" });
  }
});
...
route.get("/profile/:id", userProtected, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Internal server error on getProfile" });
  }
});
...
route.get("/suggest", userProtected, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Internal server error on suggest" });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      return res.status(500).json({
        success: false,
        message: "Internal server error on editProfile",
      });
    }
  }
);
  ...
  try {
    ...
    if (user.following.includes(id)) {
      ...
      const notification = user.following.includes(id)
        ? {
            message: `${user?.userName} follow back you. You can started message now`,
            userImage: user?.profileImage,
            id: user._id,
          }
        : {
            message: `${user?.userName} started following you. If you know follow back.`,
            userImage: user?.profileImage,
            id: user._id,
          };
    ...
    return res.status(500).json({
      success: false,
      message: "Internal server error on followOrUnfollow",
    });
  }
});
...
route.get("/check", userProtected, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Internal server error on check" });
  }
});
...
route.post("/theam", userProtected, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Internal server error on theam" });
  }
});
...
route.get("/userData/:id", userProtected, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Internal server error on userData" });
  }
});
...
route.get("/notofocation", userProtected, async (req, res) => {
  try {
    ...
    if (!user.notification) {
      return res
        .status(200)
        .json({ success: true, notification: [], notificationLength: 0 });
    }

    return res.status(200).json({
      success: true,
      notification: user.notification,
      notificationLength: user.notoficationLength,
    });
  } catch (error) {
    console.log("notofocation", error);
    return res.status(500).json({
      success: false,
      message: "Internal server error on notofocation",
    });
  }
});
...
route.post("/search", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Internal server error search route" });
  }
});

route.get("/get_user/:id", async (req, res) => {
  const { id } = req.params;
  try {
    ...
    if (!users && !user) {
      return res
        .status(200)
        .json({ success: false, message: "All users", data: [] });
    }
    return res
      .status(200)
      .json({ success: false, message: "All users", data: {users:users, user: user} });
  } catch (error) {
    return res
      .status(500)
      .json({ success: false, message: "Internal server error search route" });
